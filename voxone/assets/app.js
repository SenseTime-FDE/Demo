...
(function () {
  ...
  const logout = () => { localStorage.removeItem(TK); location.href = "/login"; };
  ...
  function shell(me, pageId) {
    ...
    for (const n of NAV) {
      ...
      nav += `<a class="sb-item ${n.id === pageId ? "on" : ""}" href="/${n.id}"><span class="ic">${n.ic}</span>${n.t}</a>`;
    }
  ...
  async function page(pageId, render) {
    ...
    if (!(me.perms.pages || []).includes(pageId)) {
      document.body.innerHTML = `<div class="empty" style="margin-top:20vh"><div class="ic">🔒</div><h3>无权访问该页面</h3><p class="sub">当前角色「${esc(me.roleLabel)}」不包含此页面。</p><a class="btn pri" href="/index" style="margin-top:12px">返回概览</a></div>`;
      return;
    }
    ...
    api("/api/rt/health").then((h) => { const em = $("#envmode"); if (em) { em.textContent = h.mode === "live" ? "实时端点 Live" : "内置引擎"; const d = $(".env .dot"); if (d) d.className = "dot " + (h.ok ? "g" : "r"); } }).catch(() => {});
  }
