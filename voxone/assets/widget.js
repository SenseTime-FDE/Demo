...
(function () {
  ...
  function invoke(text) {
    var url = CFG.base + "/api/agent-gateway/invoke?token=" + encodeURIComponent(CFG.token);
    var body = {
  ...
  function fetchCaps() {
    var url = CFG.base + "/api/agent-gateway/capabilities?token=" + encodeURIComponent(CFG.token);
  ...
  function serverSpeak(text, lang) {
    ...
    var url = CFG.base + "/api/agent-gateway/tts?token=" + encodeURIComponent(CFG.token);
    fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: text, lang: CFG.lang }) })
  ...
  function transcribe(blob, mime) {
    ...
    blobToB64(blob).then(function (b64) {
      var url = CFG.base + "/api/agent-gateway/asr?token=" + encodeURIComponent(CFG.token);
      return fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ audioBase64: b64, contentType: mime, filename: "rec.webm", lang: CFG.lang }) });
    }).then(function (r) { return r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status)); })
      .then(function (j) {
