/* ============================================================
   BaoChat · 微信语音 Agent 对话原型引擎（报社一线工作流演示）
   用法：
     BaoChat.mount(document.getElementById('phone-slot'), {
       contact: { name: 'S-Agent · 记者助理', corp: '@某某报社', sub: '日日新大模型技术底座 · 企业微信' },
       user:    { name: '李建国', avatar: '李' },
       scenarios: [ { id, title, steps: [step, ...] }, ... ]
     })
   step 结构（按 from 分流）：
     { from:'agent'|'user',
       voice:'转写文字', dur:8,          // 语音消息（自动出转写条，SenseAudio-V2 语音大模型示意）
