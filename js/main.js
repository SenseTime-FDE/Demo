...
(function () {
  ...
  function renderGrid() {
    $("#grid").innerHTML = PROJECTS.map((p) => (p.type === "qr" ? renderQrCard(p) : renderWebCard(p))).join("");
  }
  ...
  function bindInteractions() {
    ...
    // 预览 iframe 延迟加载：仅当卡片进入视口（用户主动滚到 Demo 区）才加载，
    // 避免初始加载时 iframe 自动获焦把页面"拽"到 Demo 板块。
